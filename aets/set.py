    ...
    for trace in train_list:
        for event in trace[:-1]:
            ...
            if currEventIndex >= 2:
                ...
                train_event.append(list(set(tempEvent)))
    ...
    for trace in test_list:
        for event in trace[:-1]:
            ...
            if currEventIndex >= 2:
                ...
                test_event.append(list(set(tempEvent)))
...
def train(file, ts_type, time_unit):
    ...
    stateValue = [x for x in state_dict.values()]
    ...
    for eventIndex in range(0, len(train_event)):
        currStateIndex = stateValue.index(set(train_event[eventIndex]))
    ...
    for eventIndex in range(0, len(test_event)):
        currStateIndex = stateValue.index(set(test_event[eventIndex]))
    ...
    optimizerLinear = optim.Adam(modelTotal.parameters(), lr=0.00001)
    # total model train
    ...
    for index in singleTrainVec.keys():
        ...
        criterionLinear = nn.L1Loss().cuda()
        optimizerLinear = optim.Adam(modelLinear.parameters(), lr=0.0001)
        for epoch in range(100):
            total_loss = 0
            for i, (x, y) in enumerate(my_dataset_loader):
...
def train_set(file, ts_type, time_unit):
    # train
